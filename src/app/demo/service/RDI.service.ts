...
export class RDIService {
  ...
  private baseUrl = 'http://localhost:8089/esprobackend/RDI';
  ...
  findRDIMemberByUser(userId: number): Observable<RDIMember> {
    return this.http.get<RDIMember>(`${this.baseUrl}/retrieve-User-RDIMember/${userId}`);
  }
 
  getMostSimilarRDIs(targetRDIId: number): Observable<RDI[]> {
    return this.http.get<RDI[]>(`${this.baseUrl}/most-similar`, {
      params: {
        targetRDIId: targetRDIId,
      },
    }); // Pass the target RDI ID as a query parameter
    };
  }
