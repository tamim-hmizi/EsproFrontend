...
export class LevelComponent implements OnInit {
        ...
        if (this.level.id === 0) {
            ...
            this.levelService
                ...
                .subscribe((newLevel) => {
                   ...
                   //this.level.endYear = endYear;
                    ...
                    this.levelService.retrieveStartYearAndEndYearForClassroomsByLevelId(this.level.id)
                        .subscribe((years) => {
                            this.level.startYear = years[0];
                            this.level.endYear = years[1];
                        });
                });
            location.reload();
        } else {
            ...
            this.levelService
                ...
                .subscribe((updatedLevel) => {
                    ...
                    this.levelService.retrieveStartYearAndEndYearForClassroomsByLevelId(this.level.id)
                        .subscribe((years) => {
                            this.levels[index].startYear = years[0];
                            this.levels[index].endYear = years[1];
                        });
                });
            location.reload();
    ...
    isValidLevel(): boolean {
        ...
        const existingLevel = this.levels.find(
            (val) =>
                val.name.toLowerCase() === this.level.name.toLowerCase() &&
                this.getYearFromDate(val.startYear) === this.getYearFromDate(this.level.startYear) &&
                this.getYearFromDate(val.endYear) === this.getYearFromDate(this.level.endYear) &&
                (this.level.id === 0 || val.id !== this.level.id),
        );
        if (existingLevel) {
            // Level with the same name, start year, and end year already exists
            this.messageService.add({
                severity: 'error',
                summary: 'Error',
                detail: 'A level with the same name, start year, and end year already exists',
                life: 3000,
            });
            return false;
