...
export class ClassroomComponent implements OnInit {
    ...
    editClassroom(classroom: Classroom) {
       ...
       //this.classroom.startYear = new Date(parseInt(this.classroom.startYear), 0); // Convert startYear to Date
        //this.classroom.endYear = new Date(parseInt(this.classroom.endYear), 0); // Convert endYear to Date
    ...
    saveClassroom() {
        ...
        this.resetClassroom();
        location.reload();
        // Reset classroom data after saving
    }
    ...
    generateAllClassroomSalles() {
        this.classrooms.forEach((classroom) => {
            ...
            do {
                ...
                const floor = Math.floor(Math.random() * 4) + 1;
                const room = Math.floor(Math.random() * 14) + 1;
                classroomSalle = `${bloc}${floor}${room < 10 ? `0${room}` : room}`;
            } while (
                this.classrooms.some(
                    (c) =>
                        c.classroomSalle === classroomSalle &&
                        c.startYear === classroom.startYear
                )
                );

            classroom.classroomSalle = classroomSalle;
    ...
    generateAllClassroomsModules() {
        this.moduleService.getAllModules().subscribe((modules) => {
            // Find the required modules
            const frenchModule = modules.find(module => module.name === 'French');
            const englishModule = modules.find(module => module.name === 'English');
            const mathsModule = modules.find(module => module.name === 'Maths');
            const piModule = modules.find(module => module.name === 'Pi');

            this.classrooms.forEach((classroom) => {
                // Add French and English modules to all classrooms
                classroom.modules.push(frenchModule, englishModule);

                // Add Maths module to classrooms that start with 2 or 3
                if (classroom.name.startsWith('2') || classroom.name.startsWith('3')) {
                    classroom.modules.push(mathsModule);
                }

                // Add Pi module to classrooms that start with 4
                if (classroom.name.startsWith('4')) {
                    classroom.modules.push(piModule);
                }

                // Update the classroom in the database
                this.classroomService.updateClassroom(classroom).subscribe((data) => {
                    const index = this.classrooms.findIndex((val) => val.id === classroom.id);
                    if (index !== -1) {
                        this.classrooms[index] = data;
                    }
                });
            ...
            this.messageService.add({
                severity: 'success',
                summary: 'Successful',
                detail: 'Classroom Modules Generated',
                life: 3000,
            });
        });
    }
